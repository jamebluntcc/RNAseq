...
all_quality_data <- read.delim(paste(file_path,all_files[grep('^example.+txt$',all_files)],sep = "/"),header = T,sep = "\t")
...
for(i in 1:length(qulity_data)){
  ...
  ggsave(filename = paste(file_path,paste(plot_title,'reads_quality.png',sep = '.'),sep = "/"),type="cairo-png",plot = p,width = 8,height = 6)
  ggsave(filename = paste(file_path,paste(plot_title,'reads_quality.pdf',sep = '.'),sep = "/"),plot = p,width = 8,height = 6)
}
#----
...
ggsave(filename = paste(file_path,'example.reads_quality.png',sep = '/'),type="cairo-png",plot = all_quality_data_plot,width = 10,height = 6)
