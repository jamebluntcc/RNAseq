...
run_goseq <- function(diff_genes, gene_length_df, go_anno_df, out_prefix) {
  ...
  if (dim(out_go)[1] > 0) {
    write.table(out_go,file=paste(out_prefix, 'txt', sep = '.'),quote=F,sep='\t',row.names=F)
    write.xlsx(out_go, file = paste(out_prefix, 'xlsx', sep = '.'), sheetName = "go.enrichment", append = FALSE, row.names = F)
  }else {
