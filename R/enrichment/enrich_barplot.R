...
# kegg_barplot <- function(kegg_merge_data,x_lab = 'Input Number',y_lab =
# 'Term'){ p <- ggplot(kegg_merge_data,aes(y=Input_number,x=Term,fill=label))+
# geom_bar(stat = 'identity')+geom_bar(aes(x=Term,y=expected),fill='black',stat =
# 'identity')+ geom_text(aes(label = sign),vjust=0.5,hjust = -0.2,size = 6)+
...
# guides(fill = guide_legend(title = ''))+ylab(y_lab)+xlab(x_lab)+coord_flip() p
# } go_barplot <- function(go_merge_data,x_lab = 'Input Number',y_lab = 'Term'){
# p <- ggplot(go_merge_data,aes(y=Input_number,x=Term,fill=label))+ geom_bar(stat
# = 'identity')+geom_bar(aes(x=Term,y=expected),fill='black',stat = 'identity')+
...
# facet_grid(ontology~.,scales = 'free_y',space = 'free_y')+coord_flip() p }
#----plot out-----
