...
#----for test-----
# all_count_file <- "./gga.txt"
# enrichment_type <- "go"
# enrichment_tables_path <- "./enrich/go/N7_vs_N42/"
# all_lists_path <- "./differential_analysis/N7_vs_N42/"
...
if(enrichment_type == 'kegg'){
  ...
  for(each_file in all_enrichment_files){
    if(! grepl('kegg.enrichment.txt', each_file)) next
    ...
    if(unlist(strsplit(first_level_split[1],split = "_vs_"))[1] == unlist(strsplit(first_level_split[2],split = "-"))[1]){
      ...
      up_file_list <- paste(unlist(strsplit(up_file,split = "\\."))[1],unlist(strsplit(up_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
      ...
      down_file_list <- paste(unlist(strsplit(down_file,split = "\\."))[1],unlist(strsplit(down_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
      ...
      all_file_list <- paste(unlist(strsplit(all_file,split = "\\."))[1],unlist(strsplit(all_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
  ...
  all_enrichment_files <- list.files(enrichment_tables_path)
  for(each_file in all_enrichment_files){
    if(! grepl('go.enrichment.txt', each_file)) next
    first_level_split <- unlist(strsplit(each_file,split = "\\."))
    if(unlist(strsplit(first_level_split[1],split = "_vs_"))[1] == unlist(strsplit(first_level_split[2],split = "-"))[1]){
      up_file <- each_file
      up_data <- read.delim(paste(enrichment_tables_path,up_file,sep = "/"),header = T, stringsAsFactors = F)
      up_file_list <- paste(unlist(strsplit(up_file,split = "\\."))[1],unlist(strsplit(up_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
      up_data_count_list <- read.delim(paste(all_lists_path,up_file_list,sep = "/"),header = F)
    }else if(unlist(strsplit(first_level_split[1],split = "_vs_"))[2] == unlist(strsplit(first_level_split[2],split = "-"))[1]){
      down_file <- each_file
      down_data <- read.delim(paste(enrichment_tables_path,down_file,sep = "/"),header = T, stringsAsFactors = F)
      down_file_list <- paste(unlist(strsplit(down_file,split = "\\."))[1],unlist(strsplit(down_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
      down_data_count_list <- read.delim(paste(all_lists_path,down_file_list,sep = "/"),header = F)
    }else if(first_level_split[2] == "ALL"){
      all_file <- each_file
      all_data <- read.delim(paste(enrichment_tables_path,all_file,sep = "/"),header = T, stringsAsFactors = F)
      all_file_list <- paste(unlist(strsplit(all_file,split = "\\."))[1],unlist(strsplit(all_file,split = "\\."))[2],"edgeR.DE_results.diffgenes.txt",sep = ".")
      all_data_count_list <- read.delim(paste(all_lists_path,all_file_list,sep = "/"),header = F)
    }
  }
  ...
  down_data_term_count <- find_count(down_data_count_list)
  all_data_term_count <- find_count(all_data_count_list)
  go_up_data <- enrich_data(up_data,type = 'go',label = 'up',show_num = 15,max_name_length = 90,
                              all_count = find_count(go_all_term_count),term_count = up_data_term_count)
  go_down_data <- enrich_data(down_data,type = 'go',label = 'down',show_num = 15,max_name_length = 90,
  ...
  ggsave(filename = paste(output_path,paste(first_level_split[1],"go.enrichment.barplot.pdf",sep = "."),sep = "/"),
         plot = enrich_barplot(go_merge_data,break_label1 = unlist(strsplit(first_level_split[1],split = "_vs_"))[1],
                               break_label2 = unlist(strsplit(first_level_split[1],split = "_vs_"))[2]),height = 9, width= dim(go_merge_data)[1]/4)
  ggsave(filename = paste(output_path,paste(first_level_split[1],"go.enrichment.barplot.png",sep = "."),sep = "/"),
         plot = enrich_barplot(go_merge_data,break_label1 = unlist(strsplit(first_level_split[1],split = "_vs_"))[1],
