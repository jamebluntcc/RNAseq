...
theme_onmath <- function(base_size = 14) {
  theme_bw() + theme(panel.background = element_blank(), panel.grid.minor = element_blank(),
    ...
    axis.text = element_text(color = "black", face = "bold"), axis.title = element_text(face = "bold",
      size = base_size), axis.title.x = element_text(vjust = -0.2), axis.title.y = element_text(angle = 90,
      vjust = 2), plot.title = element_text(face = "bold", size = rel(1.2),
      hjust = 0.5), legend.key = element_blank(), legend.title = element_text(face = "italic"),
...
scale_color_onmath <- function(...) {
  library(scales)
  discrete_scale("colour", "onmath", manual_pal(values = c("#386cb0", "#fdb462",
...
scale_color_onmath_dark <- function(...) {
  library(scales)
  discrete_scale("colour", "onmath_dark", manual_pal(values = c(piratepal("eternal"),
    piratepal("brave"))))
}

scale_fill_onmath <- function(...) {
  library(scales)
  discrete_scale("fill", "onmath", manual_pal(values = c("#386cb0", "#fdb462",
    "#7fc97f", "#ef3b2c", "#662506", "#a6cee3", "#fb9a99", "#984ea3", "#ffff33")),
    ...)
}
## heatmap
...
om_pca_plot <- function(plot_data, samples, outdir) {
  ...
  PCA_plot <- function(PCA_data) {
    p <- ggplot(PCA_data, aes(PC1, PC2)) + geom_point(aes(colour = Group), size = rel(3)) +
      ...
      size = rel(0.5)) + theme_onmath() + scale_color_onmath() + labs(title = "PCA",
