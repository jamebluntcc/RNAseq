...
om_heatmap <- function(plot_data, samples, outdir) {
  ...
  heatmap_width <- (length(samples$sample) -5)/3 +3
  pdf(paste(outdir, 'Diff.genes.heatmap.pdf', sep = '/'), width = 3, height = 6, onefile = F)
  pheatmap(plot_data,show_rownames=F,annotation_col = ann_color,annotation_colors = ann_colors,
           ...
           treeheight_row = 0,scale = 'row')
  dev.off()
  
  png(paste(outdir,'Diff.genes.heatmap.png', sep = '/'), width = 3, height = 6,units = 'in', res = 300)
  pheatmap(plot_data,show_rownames=F,annotation_col = ann_color,annotation_colors = ann_colors,
           annotation_legend = T,annotation_names_col = F,
           color=rev(colorRampPalette(brewer.pal(10,"RdYlGn"))(100)),
           treeheight_row = 0,scale = 'row')
...
om_boxplot <- function(plot_data, samples, outdir) {
  ...
  theme_set(theme_onmath()+theme(axis.text.x = element_text(angle = -90,color = 'black',
                                                        vjust = 0.5,hjust = 0,
                                                        size = rel(length(onmath_color)*0.6)),
                             axis.text.y = element_text(size = rel(length(onmath_color)*0.6)),
                             legend.text = element_text(size = rel(length(onmath_color)*0.4)),
                             legend.key = element_blank()))
...
om_correlation_plot <- function(plot_data, samples, outdir) {
  ...
  pdf(paste(outdir, 'Sample.correlation.heatmap.pdf', sep = '/'), width = 7, height = 6, onefile = F)
  pheatmap(sample_cor,
  ...
  png(paste(outdir, 'Sample.correlation.heatmap.png', sep = '/'), width = 7, height = 6, units = 'in', res = 300)
  pheatmap(sample_cor,
           annotation_col = ann_color,
           annotation_colors = ann_colors,
           annotation_row = ann_color,
