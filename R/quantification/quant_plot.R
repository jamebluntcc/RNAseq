## onmath plot functions for RNAseq quantification analsysi
...
theme_onmath_border <- function(base_size = 14){
  theme_bw()+
    theme(panel.background = element_blank(),
          panel.grid.minor = element_blank(),
          plot.background = element_blank(),
...
scale_color_onmath_dark <- function(...) {
  library(scales)
  discrete_scale("colour","onmath_dark",manual_pal(values = c(piratepal('eternal'), piratepal('brave'))))
}

scale_fill_onmath <- function(...){
  library(scales)
  discrete_scale("fill","onmath",manual_pal(values = c("#386cb0","#fdb462","#7fc97f","#ef3b2c","#662506","#a6cee3","#fb9a99","#984ea3","#ffff33")), ...)
}
## heatmap



my_col <- c("#386cb0","#fdb462","#7fc97f","#ef3b2c","#662506","#a6cee3","#fb9a99","#984ea3","#ffff33")
...
om_heatmap <- function(plot_data, samples, outdir) {
  ...
  #theme_set(theme_onmath()+theme(axis.text.x = element_text(size = rel(length(onmath_color)*0.2))))
  ...
  ## TODO legend size
  pdf(paste(outdir, 'Diff.genes.heatmap.pdf', sep = '/'), width = heatmap_width, height = heatmap_heigh, onefile = F)
...
om_boxplot <- function(plot_data, samples, outdir) {
#  plot_data = gene_tpm_matrix
  ...
  onmath_color <- colorRampPalette(onmath_color)(length(unique(data.m$group)))
  onmath_color_bright <- colorRampPalette(onmath_color_bright)(length(samples$sample))
  onmath_color_light <- colorRampPalette(onmath_color_light)(length(unique(data.m$group)))
  ...
  boxplot1 <- ggplot(data.m,aes(x=variable,y=value,fill=group))+
    geom_boxplot(notch = T)+guides(fill = F)+
    scale_fill_manual(values = onmath_color)+xlab("")+ylab("")
  boxplot2 <- ggplot(data.m,aes(x=variable,y=value,fill=group))+
    geom_boxplot(notch = T)+guides(fill = guide_legend(nrow = 8,title = 'group'))+
    scale_fill_manual(values = onmath_color)+xlab("")+ylab("")
  violin1 <- ggplot(data.m,aes(x=variable,y=value,fill=group))+
    geom_violin()+guides(fill = F)+
    scale_fill_manual(values = onmath_color)+xlab("")+ylab("")
  violin <- ggplot(data.m,aes(x=variable,y=value,fill=group))+
    geom_violin()+guides(fill = guide_legend(nrow = 8,title = 'group'))+
    scale_fill_manual(values = onmath_color)+xlab("")+ylab("")
  density_plot <- ggplot(data.m,aes(value, color = variable, fill = group))+
    geom_density(alpha = .4)+
    scale_fill_manual(values = onmath_color)+
    scale_color_manual(values = onmath_color_bright) +
    theme(axis.text.x = element_text(angle = 0))+
    guides(fill = guide_legend(nrow = 8,title = 'group'), color = guide_legend(nrow = 8, title = 'sample'))+xlab("")
  #----print out----
  ...
  ggsave(filename = paste(plot_output_path,'Gene_expression.boxplot.png',sep = '/'),type="cairo-png",plot = boxplot2,
         width = length(unique(data.m$variable)) * width_parameter,height = 6)
  ...
  ggsave(filename = paste(plot_output_path,'Gene_expression.density_plot.pdf',sep = '/'),plot = density_plot,
         width = length(unique(data.m$variable)) * width_parameter,height = 6)
  #----merge plot----
  p <- grid.arrange(boxplot1,violin1,density_plot,nrow = 2,
...
om_volcano_plot <- function(diff_table, compare_name, logfc, qvalue, outdir) {
  ...
  for(i in 1:dim(dpa_results)[1]){
    ...
    else if(dpa_results$logFC[i] < -(logfc) & dpa_results$FDR[i] < qvalue)
  ...
  theme_set(theme_onmath()+theme(legend.key = element_blank(),
...
om_pca_plot <- function(plot_data, samples, outdir) {
  ...
  ggsave(filename = paste(outdir,"PCA_plot.pdf",sep = "/"),plot = PCA_plot(PCA_data),width = 8,height = 6)
  ggsave(filename = paste(outdir,"PCA_plot.png",sep = "/"),plot = PCA_plot(PCA_data),width = 8,height = 6,type='cairo',dpi = 300)
...
om_correlation_plot <- function(plot_data, samples, outdir) {
  ...
  pdf(paste(outdir, 'Sample.correlation.heatmap.pdf', sep = '/'), width = heatmap_width, height = heatmap_heigh, onefile = F)
  pheatmap(sample_cor,
           annotation_col = ann_color,
           annotation_colors = ann_colors,
           annotation_row = ann_color,
  ...
  # png(paste(outdir, 'Sample.correlation.heatmap.png', sep = '/'), width = heatmap_width, height = heatmap_heigh, units = 'in', res = 300)
  png(paste(outdir, 'Sample.correlation.heatmap.png', sep = '/'), width = (heatmap_width*300), height = (heatmap_heigh*300), res = 300)
  pheatmap(sample_cor,
           annotation_col = ann_color,
           annotation_colors = ann_colors,
           annotation_row = ann_color,
