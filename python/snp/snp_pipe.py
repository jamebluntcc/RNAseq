...
Usage:
    snp_pipe.py
        ...
        --BamDir=<bam_file_directory>
        --SampleInf=<sample_inf_file>
        --Ref=<refference_fasta>
...
class add_read_groups(luigi.Task):
        ...
        def run(self):
            ...
            'O={0}/{1}.rg_added_sorted.bam'.format(each_sample_dir, self.sample),
            'SO=coordinate',
...
class mark_duplicates(luigi.Task):
    ...
    def run(self):
        ...
        PICARD_PATH,
        'MarkDuplicates',
        'I={0}/bam/{1}/{1}.rg_added_sorted.bam'.format(OutDir, self.sample),
        'O={0}/bam/{1}/{1}.dedupped.bam'.format(OutDir, self.sample),
        'M={0}/bam/{1}/{1}.metrics'.format(OutDir, self.sample),
        'CREATE_INDEX=false',
...
class split_ncigar_reads(luigi.Task):
    '''
    1. splits reads into exon segments (getting rid of Ns but maintaining grouping information) and hard-clip any sequences overhanging into the intronic regions
    ...
    def run(self):
        ...
        '-Djava.io.tmpdir={0}/bam/{1}/tmp'.format(OutDir, self.sample),
        '-jar',
        GATK_PATH,
        '-T',
        'SplitNCigarReads',
...
class snp_calling(luigi.Task):
    ...
    def run(self):
        ...
        bam_path_list = ['{0}/bam/{1}/{1}.split.bam'.format(OutDir, each_sample) for each_sample in sample_list]
...
class snp_filter(luigi.Task):
    ...
    def run(self):
        ...
        '{}/snp/snp.raw.vcf'.format(OutDir),
        '-o',
        '{}/snp/snp.filter.vcf'.format(OutDir)])
...
class snp_collection(luigi.Task):
    ...
    def requires(self):
        global OutDir, BamDir, sample_list, Ref
    ...
    def run(self):
        with self.output().open('w') as snp_collection_log:
            snp_collection_log.write('snp analysis finished')
