...
for each_sample in sample_list:
    ...
    reads_quality_dict = {}
    each_reads_quality_file = os.path.join(reads_quality_dir, '{}.reads_quality.txt'.format(each_sample))
    for n in (1, 2):
        each_qc_dir = os.path.join(qc_dir, 'fastqc_results', '%s_%s.clean_fastqc' % (each_sample, n))
    ...
    each_sample_gc_info_table = os.path.join(gc_dir, '{0}.gc.txt'.format(each_sample))
