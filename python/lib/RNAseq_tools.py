...
class RNAseq_pipeline():
    ...
    def get_cleandata(self):
        ...
        if self.sample_list:
            ...
            print self.group_sample
            self.sample_list = [each.strip().split()[1] for each in open(self.group_sample)]
    ...
    def run_quant(self):
        ...
        if self.quant_program == 'kallisto':
            quant_result_list = [os.path.join(self.quant_dir, each_sample, 'abundance.tsv.genes') for each_sample in self.sample_list]
        else:
            quant_result_list = [os.path.join(self.quant_dir, each_sample, 'quant.sf.genes') for each_sample in self.sample_list]
        quant_results_line = ' '.join(quant_result_list)
