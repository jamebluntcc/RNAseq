...
if __name__ == '__main__':
    ...
    star_log_file_list = [path.join(
        star_mapping_dir, each_sample, 'Log.final.out') for each_sample in sample_list]
    ...
    # output plot data
    plot_data = star_log_out_df.loc[:, [
        'Number of input reads', 'Uniquely mapped reads number', 'Number of reads mapped to multiple loci']]
    report_data = star_log_out_df.loc[:, [
        'Uniquely mapped reads %', '% of reads mapped to multiple loci']]
    plot_data.columns = ['total_reads',
                         'unique_mapped_reads', 'multiple_mapped_reads']
    report_data.columns = [
        'Unique_mapped_reads(%)', 'Multiple_mapped_reads(%)']
    star_mapping_stats_plot = '{}.plot'.format(star_mapping_stats_prefix)
    star_mapping_stats_report = '{}.report'.format(star_mapping_stats_prefix)
    plot_data.to_csv(star_mapping_stats_plot, sep='\t')
    report_data.to_csv(star_mapping_stats_report, sep='\t')
