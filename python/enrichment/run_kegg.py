...
class KEGG_enrich :
    ...
    def run_kegg_pathview(self):
        ...
        for each_compare in compare_list:
            ...
            diff_out_list = glob('{}/*.edgeR.DE_results.txt'.format(each_compare_diff_dir))
            for each_diff_file in diff_out_list:
                each_diff_file_name = path.basename(each_diff_file)
                each_out_prefix = each_diff_file_name.split('.edgeR.DE_results')[0]
                if 'UP' not in each_out_prefix:
                    each_out_prefix = '{}.ALL'.format(each_out_prefix)
                kegg_output = path.join(each_compare_out_dir,'%s.kegg.enrichment.txt' % (each_out_prefix))
                pathway_outdir = path.join(each_compare_out_dir,'%s.pathway' % each_out_prefix)
                pathview_check_log_file = path.join(pathway_log_dir,'%s.log' % (each_out_prefix))
                pathview_cmd = 'python %s --kegg_table %s --blast_out %s --species %s --diff_out %s --out_dir %s' % (PATHVIEW, kegg_output, self.all_blast_out, self.species, each_diff_file, pathway_outdir)
    ...
    def run_KEGG_enrich(self):
        ...
        for each_compare in compare_list :
            each_compare_diff_dir = path.join(self.diff_dir, each_compare)
            diff_gene_list = glob('{}/*.diffgenes.txt'.format(each_compare_diff_dir))
            each_compare_out_dir = path.join(self.out_dir,each_compare)
            ...
            for each_diff_file in diff_gene_list:
                ...
                each_out_prefix = each_diff_file_name.split('.edgeR.DE_results')[0]
                kegg_output = path.join(each_compare_out_dir,'%s.kegg.enrichment.txt' % (each_out_prefix))
                each_blast_out = path.join(blast_out_dir,'%s.blasttab' % (each_out_prefix))
        ...
        pathway_cmd = self.run_kegg_pathview()
        pathway_cmd.extend(pathway_cmd)
        return cmd_list
